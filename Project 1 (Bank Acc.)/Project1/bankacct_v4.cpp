...
*/		cout << "argc = " << argc << endl;
		...
		read_file(fname, info, num_accts); // argc need to run command lines
		Display(cout, info, num_accts, flag);
...
//			Withdrawl();
			break;
		case 4:
			Display(cout, s, num_accts, true);	//just show one for final syntax
			break;
		case 5:
//			Transfer();
			break;
...
NOTES:             Also outputs data input into a file when called from 
	           output function
------------------------------------------------------------------------------- */
void Display(ostream &out, AcctInfo s[], int &num_accts, bool mode)
{
	//bool mode = false
