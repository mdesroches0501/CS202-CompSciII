...
Michael Des Roches 2018-03-08		  6.0 // final touchups before turn in
----------------------------------------------------------------------------- */

#include "bankacct_v6.h" // Includes header file
...
//			Withdrawl();
			break;
		case 4:
			if(validate(s, temp, true, num_accts, cls));
...
bool validate(AcctInfo *s, int &temp, bool mode, int num_accts, char *cls)
{
	...
	else
	{
		...
		for (int i = 0; i < num_accts; i++)
		{
			if(strcmp(s[temp].acctn, s[i].acctn) && strcmp(s[temp].acctp, s[i].acctp))
			{
			match = true;
			temp = i;
			}	
